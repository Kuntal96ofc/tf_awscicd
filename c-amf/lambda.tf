...
resource "aws_cloudwatch_log_group" "oai_amf_log" {
  name = "oai_amf_build"
}

resource "aws_cloudwatch_log_stream" "oai_amf_logstream" {
  name           = "oai_amf_logstream"
  log_group_name = aws_cloudwatch_log_group.oai_amf_log.name
}

resource "aws_cloudwatch_log_group" "oai_amf_sonar_log" {
  name = "oai_amf_sonar_build"
}

resource "aws_cloudwatch_log_stream" "oai_amf_sonar_logstream" {
  name           = "oai_amf_sonar_logstream"
  log_group_name = aws_cloudwatch_log_group.oai_amf_sonar_log.name
}
