//IAM Roles Creation for oai-amf module

resource "aws_iam_role" "amf_build" {
  name = "amf-build-terraform"
...
resource "aws_iam_role_policy" "AmazonEC2ContainerRegistryFullAccess" {
  role = aws_iam_role.amf_build.name
    ...
    "Statement": [
        {
            "Effect": "Allow",
            "Resource": [
                "arn:aws:logs:<AWSRegion>:*:log-group:/aws/codebuild/oai_amf_build",
                "arn:aws:logs:<AWSRegion>:*:log-group:/aws/codebuild/oai_amf_build:*"
            ],
            "Action": [
                "logs:CreateLogGroup",
        ...
        },
        {
            "Effect": "Allow",
            "Resource": [
                "arn:aws:codecommit:<AWSRegion>:*:oai-amf"
            ],
            "Action": [
                "codecommit:GitPull"
            ]
        },
        {
            ...
            "Resource": [
                "arn:aws:codebuild:<AWSRegion>:*:report-group/oai-amf-terraform-*"
            ]
        }
    ]
}
...
//IAM Roles Creation for oai-amf-sonar module

resource "aws_iam_role" "amf_sonar_build" {
  name = "amf-sonar-terraform"
...
resource "aws_iam_role_policy" "AmazonS3FullAccessforSonarQube" {
  role = aws_iam_role.amf_sonar_build.id
...
resource "aws_iam_role_policy" "CodeBuildBasePolicy-oai-amf-sonarbuild" {
  role = aws_iam_role.amf_sonar_build.id
...
resource "aws_iam_role" "amf_pipeline_iam" {
  name = "amf-pipeline-terraform"
...
resource "aws_iam_role_policy" "amf_pipeline_iam" {
  role = aws_iam_role.amf_pipeline_iam.name
